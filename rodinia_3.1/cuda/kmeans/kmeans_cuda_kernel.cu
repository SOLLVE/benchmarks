...
__global__ void invert_mapping(float *input,			/* original */
							   ...
							   unsigned long long npoints,				/* npoints */
							   int nfeatures)			/* nfeatures */
{
	unsigned long long point_id = threadIdx.x + blockDim.x*blockIdx.x;	/* id of thread */
	int i;
			...
			int    *block_deltas) 
{
	...
	// point/thread ID  
	const unsigned long long point_id = block_id*blockDim.x*blockDim.y + threadIdx.x;
