...
void BFSGraph( int argc, char** argv) 
{
	...
	if(numNodesPerDev>MAX_THREADS_PER_BLOCK)
	{
		num_of_blocks = (unsigned long long)ceil(numNodesPerDev/(double)MAX_THREADS_PER_BLOCK); 
    ...
    if (edge_um_size)
      ...
      cudaMemAdvise(h_graph_edges_2, sizeof(unsigned long long)*edge_um_size, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId);
#endif
#else
	...
	do
	{
		...
		*d_over=false;
		...
		Kernel<<< grid, threads, 0 >>>( h_graph_nodes, h_graph_edges, h_graph_mask, h_updating_graph_mask, h_graph_visited, h_cost, no_of_nodes, start);
#elif defined (CUDA_HYB) && defined (CUDA_HYB_L)
...
#elif defined (CUDA_HYS)
		Kernel<<< grid, threads, 0 >>>( h_graph_nodes, h_graph_edges, d_graph_mask, d_updating_graph_mask, h_graph_visited, h_cost, no_of_nodes, start);
#else
		Kernel<<< grid, threads, 0 >>>( d_graph_nodes, d_graph_edges, d_graph_mask, d_updating_graph_mask, d_graph_visited, d_cost, no_of_nodes, start);
#endif
        }
        for (int d = 0; d < numDevice; d++) {
          cudaSetDevice(d);
        ...
        for (int d = 0; d < numDevice; d++) {
          ...
          unsigned long long start = d * numNodesPerDev;
#if defined (CUDA_UVM) || defined (CUDA_HST)
		Kernel2<<< grid, threads, 0 >>>( h_graph_mask, h_updating_graph_mask, h_graph_visited, d_over, no_of_nodes, start);
#elif defined (CUDA_HYB) && defined (CUDA_HYB_L)
		Kernel2<<< grid, threads, 0 >>>( d_graph_mask, d_updating_graph_mask, h_graph_visited, d_over, no_of_nodes, start);
#elif defined (CUDA_HYB)
		Kernel2<<< grid, threads, 0 >>>( d_graph_mask, d_updating_graph_mask, d_graph_visited, d_over, no_of_nodes, start);
#elif defined (CUDA_HYS)
		Kernel2<<< grid, threads, 0 >>>( d_graph_mask, d_updating_graph_mask, h_graph_visited, d_over, no_of_nodes, start);
#else
		Kernel2<<< grid, threads, 0 >>>( d_graph_mask, d_updating_graph_mask, d_graph_visited, d_over, no_of_nodes, start);
