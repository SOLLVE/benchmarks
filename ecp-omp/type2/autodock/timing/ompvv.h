...
// Macro for checking if offloading is enabled or not and set a variable with
// the result
...
// Macro for setting errors on condition and displaying an error if something
// went wrong
...
#define OMPVV_TEST_SHARED_ENVIRONMENT_PROBE                                    \
  ...
  int _ompvv_isOffloadingOn = 0;                                               \
  _Pragma("omp target map (from: _ompvv_isOffloadingOn) map(to: "              \
          "_ompvv_isSharedEnv)") {                                             \
    _ompvv_isOffloadingOn = !omp_is_initial_device();                          \
...
#define OMPVV_TEST_SHARED_ENVIRONMENT                                          \
  {                                                                            \
    ...
    OMPVV_WARNING_IF((_ompvv_isOffloadingOn && _ompvv_isSharedEnv == 0),       \
                     "This tests is running on a shared data environment "     \
                     "between host and device. This may cause errors")         \
  }

// Macro to report warning if it is a shared environment and set a variable for
// further use
