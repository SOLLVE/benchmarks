...
__global__ void BlockSpmvKernel(const unsigned int A_num_rows,
                                IndexType* A_row_offsets,
                                IndexType* A_column_indices,
                                ValueType* A_values,
                                size_t A_ci_device_size,
                                size_t A_v_device_size,
                                IndexType* A_column_indices_sec,
                                ValueType* A_values_sec,
                                ValueType* X_values,
                                ValueType* Y_values,
                                ...
                                BinaryFunction2 reduce)
{
    ...
    for(IndexType row = vector_id; row < A_num_rows; row += num_vectors)
    {
        ...
        for(IndexType jj = row_start; jj < row_end; jj += THREADS_PER_VECTOR)
        {
            ...
            {
                //sdata[threadIdx.x] = A_column_indices[jj + thread_lane];
                if(jj + thread_lane < A_ci_device_size)
                    sdata[threadIdx.x] = A_column_indices[jj + thread_lane];
                else
                    sdata[threadIdx.x] = A_column_indices_sec[jj + thread_lane - A_ci_device_size];
                //vdata[threadIdx.x] = A_values[jj + thread_lane];
                if(jj + thread_lane < A_v_device_size)
                    vdata[threadIdx.x] = A_values[jj + thread_lane];
                else
                    vdata[threadIdx.x] = A_values_sec[jj + thread_lane - A_v_device_size];
            }
                      ...
                      BinaryFunction2 reduce)
{
    ...
    // lld: modify to manually allocated storage
    const size_t MAX_BLOCKS = cusp::system::cuda::detail::max_active_blocks(
                                  BlockSpmvKernel<IndexType, ValueType,
                                  UnaryFunction, BinaryFunction1, BinaryFunction2,
                                  VECTORS_PER_BLOCK, THREADS_PER_VECTOR>,
                                  THREADS_PER_BLOCK, (size_t) 0);
    const size_t NUM_BLOCKS = std::min(20 * MAX_BLOCKS, DIVIDE_INTO(A.num_rows, VECTORS_PER_BLOCK));
    //const size_t NUM_BLOCKS = std::min(20 * 60*(2048 / THREADS_PER_BLOCK), DIVIDE_INTO(A.num_rows, VECTORS_PER_BLOCK));
    ...
    // lld: modify to manually allocated storage
    BlockSpmvKernel<IndexType, ValueType, //ValueIterator2, ValueIterator3,
                    UnaryFunction, BinaryFunction1, BinaryFunction2,
                    VECTORS_PER_BLOCK, THREADS_PER_VECTOR>
      <<<NUM_BLOCKS, THREADS_PER_BLOCK, 0, s>>>
      (A.num_rows, A.row_offsets.my_begin, A.column_indices.my_begin, A.values.my_begin,
       A.column_indices.my_device_size, A.values.my_device_size, A.column_indices.my_second_begin, A.values.my_second_begin,
       x.values.my_begin, y.values.my_begin,
       initialize, combine, reduce);
}
