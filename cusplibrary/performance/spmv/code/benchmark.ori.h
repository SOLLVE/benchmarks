...
//#define CUDA_UM_PREFERRED_BOTH
//#define CUDA_HOST_ALLOC
//#define CUDA_HOST_ACCESSEDBY
//#define CUDA_VEC_PINNED
...
float time_spmv_block(TestMatrix& test_matrix, size_t num_cols, TestKernel test_spmv, double seconds = 3.0, size_t min_iterations = 100, size_t max_iterations = 500)
{
...
#elif defined (CUDA_UM_ALLOC)
    cudaMallocManaged((void **)&test_x.values.my_begin, N*num_cols*sizeof(ValueType));
    cudaMallocManaged((void **)&test_y.values.my_begin, M*num_cols*sizeof(ValueType));
    //for(unsigned long long i = 0; i < N*num_cols; i++)
    //    test_x.values.my_begin[i] = (ValueType)0;
    //for(unsigned long long i = 0; i < M*num_cols; i++)
    //    test_y.values.my_begin[i] = (ValueType)0;
#elif defined (CUDA_HOST_ALLOC)
    cudaMallocHost((void **)&test_x.values.my_begin, N*num_cols*sizeof(ValueType));
    cudaMallocHost((void **)&test_y.values.my_begin, M*num_cols*sizeof(ValueType));
    //for(unsigned long long i = 0; i < N*num_cols; i++)
    //    test_x.values.my_begin[i] = (ValueType)0;
    //for(unsigned long long i = 0; i < M*num_cols; i++)
    //    test_y.values.my_begin[i] = (ValueType)0;
#else
    cudaMalloc((void **)&test_x.values.my_begin, N*num_cols*sizeof(ValueType));
    cudaMalloc((void **)&test_y.values.my_begin, M*num_cols*sizeof(ValueType));
    //ValueType* host_test_x = new ValueType[N*num_cols];
    //ValueType* host_test_y = new ValueType[M*num_cols];
    //for(unsigned long long i = 0; i < N*num_cols; i++)
    //    host_test_x[i] = (ValueType)0;
    //for(unsigned long long i = 0; i < M*num_cols; i++)
    //    host_test_y[i] = (ValueType)0;
    //cudaMemcpy(test_x.values.my_begin, (void *)host_test_x, N*num_cols*sizeof(ValueType), cudaMemcpyHostToDevice);
    //cudaMemcpy(test_y.values.my_begin, (void *)host_test_y, M*num_cols*sizeof(ValueType), cudaMemcpyHostToDevice);
#endif
    ...
    //cudaMemAdvise(test_matrix.row_offsets.my_begin, (test_matrix.num_rows+1)*sizeof(IndexType), cudaMemAdviseSetAccessedBy, global_device_id);
    //cudaMemAdvise(test_matrix.column_indices.my_begin, test_matrix.num_entries*sizeof(IndexType), cudaMemAdviseSetAccessedBy, global_device_id);
...
void test_csr(HostMatrix& host_matrix)
{
  ...
  #define test_matrix_on_host host_matrix
    ...
    printf("\t%-20s: %8.4f ms\n", "cudaMallocManaged", elapsed_time);
...
#elif defined (CUDA_UM_PREFERRED_BOTH)
    cudaMemAdvise(test_matrix_on_device.row_offsets.my_begin, (test_matrix_on_device.num_rows+1)*sizeof(IndexType), cudaMemAdviseSetPreferredLocation, global_device_id);
    cudaMemAdvise(test_matrix_on_device.column_indices.my_begin, test_matrix_on_device.num_entries*sizeof(IndexType), cudaMemAdviseSetPreferredLocation, global_device_id);
    cudaMemAdvise(test_matrix_on_device.values.my_begin, test_matrix_on_device.num_entries*sizeof(ValueType), cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId);
    cudaMemAdvise(test_matrix_on_device.values.my_begin, test_matrix_on_device.num_entries*sizeof(ValueType), cudaMemAdviseSetAccessedBy, global_device_id);
...
#elif defined (CUDA_HOST_ALLOC)
    cudaEventRecord(start,0);
    cudaMallocHost((void **)&test_matrix_on_device.row_offsets.my_begin, (test_matrix_on_device.num_rows+1)*sizeof(IndexType));
    cudaMallocHost((void **)&test_matrix_on_device.column_indices.my_begin, test_matrix_on_device.num_entries*sizeof(IndexType));
    cudaMallocHost((void **)&test_matrix_on_device.values.my_begin, test_matrix_on_device.num_entries*sizeof(ValueType));
    cudaEventRecord(end, 0);
    cudaEventSynchronize(end);
    cudaEventElapsedTime(&elapsed_time, start, end);
